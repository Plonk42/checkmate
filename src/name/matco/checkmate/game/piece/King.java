...
public class King extends Piece {
	...
	public List<List<Movement>> getAllowedMovements() {
		...
		if (isKingSideCastlingValid) {
			for (int i = kingCorner.getIndex(); i <= getSquare().getIndex() - 1; i++) {
				final Square s = getSquare().getBoard().getSquareAt(i);
				if (!s.isEmpty()) {
					isKingSideCastlingValid = false;
					break;
				}
		...
		if (isQueenSideCastlingValid) {
			for (int i = getSquare().getIndex() + 1; i <= queenCorner.getIndex() - 1; i++) {
				final Square s = getSquare().getBoard().getSquareAt(i);
				if (!s.isEmpty()) {
					isQueenSideCastlingValid = false;
					break;
